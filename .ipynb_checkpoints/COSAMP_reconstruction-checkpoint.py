...
def Fourier(samples, values, frequencies, num_iters=5000, tol=1e-5, lstsq_rec=False):
    ...
    def col_extractor(samples, frequencies, max_ind):
        ...
        mat = (samples.to(torch.double) * frequencies[ind_x,:].to(torch.double)).sum(-1)
        if ind_y==0:
            mat = torch.cos(mat)
        else:
            mat = torch.sin(mat)     
        return mat[:,None].to(dtype)/np.sqrt(N)
    ...
    rec = OMP(col_extractor, AT, normalization, b, frequencies, samples, num_iters=num_iters, tol=tol)
...
def Tchebychev(samples, values, indices, num_iters=5000, tol=1e-5, tensor=False, lstsq_rec=False):
    ...
    if tensor:
        A_Mat = norm_coeffs * torch.cos(indices[:, None, :] * samples_acos[None, :, :]).prod(-1)
    else:
        A_op = Tchebychev_eval(samples_acos, indices, norm_coeffs, D)
        AT_op = aTchebychev_eval(samples_acos, indices, norm_coeffs, D)
    ...
    # run OMP reconstruction
    rec = OMP(col_extractor, AT, normalization, b, indices, samples_acos, num_iters=num_iters, tol=tol)
    residuals = torch.zeros((2,), dtype=dtype, device = device)
    residuals[0] = torch.linalg.norm(A(rec) - b)
    if lstsq_rec:
        residuals[1] = torch.linalg.norm(A(lsr_rec) - b)
...
def OMP(col_extractor, AT, normalization, b, f, p, num_iters=1500, tol=1e-5):
    ...
    for j in range(num_iters):
        ...
        if j == 0:
            ...
            new_col = col_extractor(p, f, max_ind)
            v = torch.linalg.solve_triangular(L[:j,:j], A_mat[:,:j].transpose(0,1) @ new_col, upper=False)
            L[j, :j] = v.T
            L[j, j] = torch.sqrt(
...
def aTchebychev_eval(p_acos, k, pre, D):
    # Non-Uniform Discrete Fourier Transform (multidimensional)
    #   x : tensor of type torch.Tensor and shape (N,1), real valued
...
def Tchebychev_eval(p_acos, k, pre, D):
    # Non-Uniform Discrete Fourier Transform (multidimensional)
    #   x : tensor of type torch.Tensor and shape (N,2), real-valued
    #   p, f : tensors of type torch.Tensor and shapes (N,D)
    k_j = Vj(k)  # (M, 1, D) LazyTensor
    pre_j = Vj(pre)  # (M, 1, 1) LazyTensor
    p_acos_i = Vi(p_acos)  # (1, N, D) LazyTensor
    x_j = Vj(0, 1)
