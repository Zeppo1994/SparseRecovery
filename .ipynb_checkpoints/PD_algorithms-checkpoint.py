...
def pd_rLASSO(
    ...
    x, x_old, y = x0, x0, y0
    Xavg, Yavg = torch.clone(x0), torch.clone(y0)
    vals, vals_dual = [], []
    ...
    for j in iterator:
        ...
        vals.append(
            (
                torch.linalg.norm(A(Xavg) - b)
                + lam * torch.sum(torch.abs(Xavg))
            ).item()
        )
        # lower bound dual objective by constructing feasible y
        dual_arg = Yavg * torch.clamp(
            lam / torch.max(torch.abs(AT(Yavg))), max=1.0
        )
    ...
    # modify output if final iterates are better than average
    final_energy_primal = torch.linalg.norm(A(x) - b) + lam * torch.sum(
        torch.abs(x)
    )
    if final_energy_primal < vals[-1]:
        Xavg = x
        vals[-1] = final_energy_primal

    y_feas = y * torch.clamp(lam / torch.max(torch.abs(AT(y))), max=1.0)
    final_energy_dual = -torch.sum(y_feas * b)
    if final_energy_dual > vals_dual[-1]:
        dual_arg = y_feas
        vals_dual[-1] = final_energy_dual
    return Xavg, dual_arg, vals, vals_dual
...
def restart_ls_pd_rLASSO(x0, A, AT, b, lam=1.0, schedule_length=15000, device_id='cuda', **kwargs,):
