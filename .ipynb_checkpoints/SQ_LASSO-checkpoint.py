...
import PD_algorithms
from deepinv. optim.utils import least_squares

def Reconstruction_Fourier(samples, values, frequencies, restarts = 9, beta=2.0, alpha=1.0, lstsq_rec=False):
    ...
    lam_est = 1. / math.sqrt(N)
    s0 = torch.zeros_like(AT(b), requires_grad=False)

    # run prim dual algorithm with restarts
    rec, vals, vals_dual = PD_algorithms.restart_pd_rLASSO(s0, A, AT, b, lam=lam_est, tol=1e-5, restarts=restarts, beta=beta, alpha=alpha)
...
def Reconstruction_Tchebychev(samples, values, indices, restarts = 9, beta=3.0, alpha=0.3, tensor=False, lstsq_rec=False):
    D = samples.size(1)  # dimension of sample points
    N = samples.size(0)  # number of sample points
    dtype = samples.dtype
    device = samples.device
    ...
    if tensor:
        A_Mat = norm_coeffs * torch.cos(indices[:, None, :] * torch.acos(samples[None, :, :])).prod(-1)
    else:
